...
INPUT_SOURCE_SIZE = os.environ.get("PAIRPUTER_INPUT_SOURCE_SIZE", "640x400")
...
# Idle-takeover: after this many seconds of NO human input, the AI auto-engages the autopilot
# (hunt/fight/advance) so the game stays alive when the human steps away. Any human input instantly
# resets the idle clock and the "human always wins" revoke hands control straight back. Scott's spec: 20s.
AUTOPILOT_IDLE_S = float(os.environ.get("PAIRPUTER_AUTOPILOT_IDLE_S", "20"))
...
class Arbiter:
    ...
    def __init__(self):
        ...
        self._start_t = time.time()  # session start — used so autopilot can engage on an untouched demo
        ...
        # Idle-assist master switch (toggleable from chat via the bridge). When off, the
        # supervisor never auto-engages on idle — the human plays solo until they turn it
        # back on. Defaults from env so a deploy can ship it off.
        self._autopilot_enabled = os.environ.get("PAIRPUTER_AUTOPILOT_ENABLE", "true").lower() not in ("", "0", "false", "no")
    ...
    def human_idle_seconds(self):
        """Seconds since the last human input (large if never / long idle). Drives idle-takeover."""
        with self._lock:
            if not self._last_human:
                return None  # no human has EVER acted this session — treat as not-yet-idle-measured
            return time.time() - self._last_human

    def snapshot(self):
        now = time.time()
        with self._lock:
            # Idle time = since last human input, OR since session start if no human has
            # EVER acted (an untouched demo should start playing itself after the threshold,
            # not sit dead waiting for a human who never arrives).
            idle_s = (now - self._last_human) if self._last_human else (now - self._start_t)
            # Idle-takeover signal: after AUTOPILOT_IDLE_S of no human input, the AI should
            # auto-engage (the in-capsule autopilot supervisor watches this). Any human event
            # resets _last_human and instantly drops it back below the threshold — the existing
            # "human always wins" revoke handles the handback mid-action.
            return {
                ...
                "humanIdleMs": int(idle_s * 1000) if idle_s is not None else None,
                "humanEverActed": bool(self._last_human),
                "autopilotIdleThresholdMs": int(AUTOPILOT_IDLE_S * 1000),
                "autopilotEnabled": self._autopilot_enabled,
                # Only SUGGEST takeover when idle-assist is enabled — the chat toggle
                # ("disable idle assist") flips this off and the human plays solo.
                "autopilotSuggested": bool(self._autopilot_enabled and idle_s is not None and idle_s >= AUTOPILOT_IDLE_S),
    ...
    class H(http.server.BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            try:
                ...
                # Idle-assist toggle from chat: POST /autopilot {enabled: true|false}.
                if self.path.rstrip("/").endswith("/autopilot"):
                    en = arb.set_autopilot_enabled(bool(body.get("enabled", True)))
                    self._json(200, {"ok": True, "autopilotEnabled": en})
                    return
...
def _map_xy(x, y):
    source = _parse_size(INPUT_SOURCE_SIZE)
    rw, rh = _root_size()
    x, y = int(x), int(y)
    if source and source != (rw, rh):
        sw, sh = source
        x = round(x * rw / sw)
        y = round(y * rh / sh)
    return max(0, min(rw - 1, x)), max(0, min(rh - 1, y))
...
def _handle_move(msg):
    x, y = _map_xy(msg.get("x", 0), msg.get("y", 0))
...
async def main():
    ...
    AUDIT("listening on %s:%s (state on :%s, agent cooldown %.2fs)" % (HOST, PORT, STATE_PORT, AGENT_COOLDOWN_S))
