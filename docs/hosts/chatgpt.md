...
- **Registration method:** `User-Defined OAuth Client` (DCR/CIMD will show as unavailable —
  expected; Cognito supports neither, and neither is required).
...
- **Display modes:** inline / fullscreen / **PiP** via `window.openai.requestDisplayMode({mode})` —
  negotiated, user-gesture-only, PiP → fullscreen on mobile; transitions may REMOUNT the widget
